 ...
 * <p>
 * * 源码分析
 * 1.底层数据结构:是一个没有被final修饰的数组，在JDK8.0之前是字符数字，在JDK9.0之后是字节数组。
 * 2.底层的初始长度：取决于使用的构造器
 * *       public StringBuilder(String str)      数组初始长度：str.length+16
 * *       public StringBuilder(CharSequence seq)数组初始长度：str.length+16
 * *       public StringBuilder(int capacity)    数组初始长度：通过参数capacity自定义数组大小
 * *       public StringBuilder() *              数组初始长度：16
 * *
 * * 3.自动扩容规则
 ...
 * 在Java语言中所有的""都是字符串的对象
 * 2.字符串本身是一个常量；它们的值在创建之后不能更改。
 * 字符串底层数据结构是数组，该数组被final修饰
 * 3.JDK8.0之前底层数据结构是一个字符数组；
 * JDK9.0之后底层数据结构是一个字节数组；
 * 4.为什么要转换成字节数组？
 * 字节在理论上的空间比字符串减少了一半，字符还要转成字节码文件，字节和二进制位转换效率比字符效率高。
 * 5.为什么字符串类型可以保存汉字？
 * String 代表UTF-16的字符串
 * 6.“” 和null区别
 * ""代表字符串的实例，可以通过"".进行方法的调用。
 * String str=new String(null)编译报错，不可实例化字符串对象
 * null是一个字面值常量，作为引用数据类型的的默认值存在，无论是对象还是数组，
 * 通过null访问对象或数组中的属性和方法都会空指针异常。
 * <p>
